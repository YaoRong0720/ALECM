...
% Extract SC data
BD_SC = SC_cell{1}; % Structural connectivity for BD
...
for num = 1:numel(fieldnames(HC_BOLD))
    name = sprintf("HC_sub%d_BOLD",num);
    sum_signal = sum_signal + getfield(HC_BOLD, name)';
end
...
% Save results to the same directory as SC_cell and BOLD_cell
save('Hetero_state_energy.mat', 'result_energy');
